...
public class EliasHelper
{
	...
	/// </summary>
	public EliasHelper(string file) : this(Path.Combine(Application.streamingAssetsPath, file), null, GetPath(Path.Combine(Application.streamingAssetsPath, file)), (uint)AudioSettings.outputSampleRate, 2, 1024, null)
	{
	}
	...
	public uint GetBusIndex(string busName)
	{
		...
		LogResult(r, "Failed to get bus index");
		return busIndex;
	}
	...
	public uint GetTransitionPresetIndex(string transitionPresetName)
	{
		...
		LogResult(r, "Failed to get transition preset index");
